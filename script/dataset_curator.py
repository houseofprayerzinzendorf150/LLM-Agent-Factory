...
# Updated: agents are now in agents_database directory
...
# Updated: new structure has all files in agents_database directly, no subfolders
DATASET_FOLDERS = ["agents_all"]  # Process all files in agents_database
...
async def call_llm(llm: ChatOpenAI, system_prompt: str, user_prompt: str) -> dict | None:
    ...
    for attempt in range(MAX_RETRIES + 1):
        ...
        try:
            ...
            if result:
                log_llm(
                    f"[{req_id}] PARSED OK: missing={len(result.get('missing_agents', []))} dups={len(result.get('duplicate_groups', []))} edits={len(result.get('agents_to_edit', []))}"
                )
                return result
...
async def process_domain_file(
    ...
    try:
        ...
        log(
            f"Domain '{domain}': LLM returned - missing={missing_count}, duplicates={dup_count}, edits={edit_count}, notes={analysis.get('notes', '')[:100]}"
        )
                ...
                for j, other in enumerate(role_agents):
                    ...
                    if sim > 0.7:
                        ...
                        if len(other.get("description", "")) > len(agent.get("description", "")):
                            ...
                            should_keep = False
                            log(
                                f"Domain '{domain}': Merged '{agent.get('agent_id')}' into '{other.get('agent_id')}' (similarity={sim:.2f})"
                            )
                            stats["agents_merged"] += 1
                            break
                        merged_ids.add(other.get("agent_id"))
                        log(
                            f"Domain '{domain}': Merged '{other.get('agent_id')}' into '{agent.get('agent_id')}' (similarity={sim:.2f})"
                        )
                        stats["agents_merged"] += 1
        ...
        log(
            f"Domain '{domain}': Curation stats - original={original_count}, final={len(final_agents)}, added={len(missing_agents)}, removed={len(removed_ids)}, merged={len(merged_ids)}"
        )
...
async def process_folder(
    ...
    # Updated: new structure has all files directly in agents_database, no subfolders
    input_folder = AGENTS_DIR if folder_name == "agents_all" else AGENTS_DIR / folder_name
    ...
    console.print(
        f"[bold green]✓ Folder '{folder_name}' complete: {processed_count}/{total_count} domains[/bold green]"
    )
...
async def curate_all_async(
    ...
    # Load catalogs
    _domains, roles, tools = load_catalogs()
    ...
    log("=" * 80)
    log(
        f"Curation complete: domains={stats['processed_domains']} +{stats['agents_added']} -{stats['agents_removed']} ~{stats['agents_merged']} ✎{stats['agents_edited']} errors={stats['errors']}"
    )
    log("=" * 80)
...
Examples:
  ...
  python dataset_curator.py --folders agents_eng
